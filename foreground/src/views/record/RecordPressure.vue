    ...
    <el-card class="page-container" v-if="isTest">
        ...
        <el-row>
            ...
            <el-form-item label="分析和建议">
                <div class="message-text mine">{{ pressure.result }}</div>
            </el-form-item>
        </el-row>
        <!-- 咨询抽屉 -->
...
import { getPressureService, addPressureService, UpdatePressureService, chatService, getHistoryService, countService, getRecordService } from '@/api/record.js';
/*体检记录 */
const records = ref([]);
const goBack = async () => {
    try {
        records.value.splice(0, records.value.length);  //清空数组
        let result = await getPressureService();
...
const goRecord = async () => {
    try {
        let result = await getRecordService("tb_pressure");
